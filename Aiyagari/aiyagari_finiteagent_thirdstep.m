function [V,c,Va_Upwind] = aiyagari_finiteagent_thirdstep(A_sim,Z_sim,V0,c_reverse_in,dA_V_DL,s_ind,data_a_index,data_z_index,params)
...
r_sim = alpha * K^(alpha-1) -delta; %interest rates
w_sim = (1-alpha) * K^(alpha);          %wages
%v = repmat(V0,[1 1 H]); 
dist = zeros(1,maxit);
% Initialization
...
% Inner Loop 
   for n=1:maxit
       g_dl = dA_V_DL .* reshape(repmat(sum(A_sim*r_sim + Z_sim*w_sim - c_sim),[I*J 1]),[I J]);
        V = v;
        % forward difference
...
c_reverse_test = zeros(N_agent,1); 
